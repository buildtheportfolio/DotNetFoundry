const API = 'https://api.github.com/repos/buildtheportfolio/DotNetFoundry/contents/projects';
...
function render() {
  ...
  projectsEl.innerHTML = visible.length ? visible.map(project => `<a class="card" href="projects/${encodeURIComponent(project.slug)}/"><h2>${project.name}</h2><p>.NET and C# project.</p><span class="open">Open project →</span></a>`).join('') : '<div class="empty">No projects match your search.</div>';
